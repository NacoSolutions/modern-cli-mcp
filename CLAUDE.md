...
Modern CLI MCP is a Model Context Protocol server written in Rust that exposes modern command-line utilities to AI/LLM agents. It bundles 25+ CLI tools (eza, bat, fd, rg, jq, etc.) and provides structured JSON-RPC access to them.
...
4. **Plain Text Output**: Most tools return plain text. Some return structured JSON when appropriate.
