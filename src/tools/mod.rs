...
pub struct XsvRequest {
    #[schemars(description = "qsv subcommand: stats, select, search, sort, slice, frequency, count, headers")]
    pub command: String,
...
pub struct UsqlRequest {
    #[schemars(description = "Database URL (e.g., postgres://user:pass@host/db, mysql://, sqlite:)")]
    pub url: String,
...
pub struct GrexRequest {
    #[schemars(description = "Test strings that the regex should match (one per line or comma-separated)")]
    pub input: String,
...
pub struct OuchCompressRequest {
    ...
    pub files: String,
    #[schemars(description = "Output archive path (extension determines format: .tar.gz, .zip, .7z, etc.)")]
    pub output: String,
}
...
impl ModernCliTools {
    ...
    #[tool(description = "List directory contents with eza (modern ls replacement). \
        Features: icons, git integration, tree view, extended attributes.")]
    async fn eza(&self, Parameters(req): Parameters<EzaRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec!["--color=never".into()];

        if req.all.unwrap_or(false) { args.push("-a".into()); }
        if req.long.unwrap_or(false) { args.push("-l".into()); }
        if req.tree.unwrap_or(false) { args.push("--tree".into()); }
        if req.git.unwrap_or(false) { args.push("--git".into()); }
        if req.icons.unwrap_or(false) { args.push("--icons".into()); }
        if req.dirs_only.unwrap_or(false) { args.push("-D".into()); }
        if req.files_only.unwrap_or(false) { args.push("-f".into()); }
        if req.reverse.unwrap_or(false) { args.push("-r".into()); }
        if let Some(level) = req.level { args.push(format!("--level={}", level)); }
        if let Some(ref sort) = req.sort { args.push(format!("--sort={}", sort)); }
        if let Some(ref path) = req.path { args.push(path.clone()); }

        let args_ref: Vec<&str> = args.iter().map(|s| s.as_str()).collect();
        match self.executor.run("eza", &args_ref).await {
            Ok(output) => Ok(CallToolResult::success(vec![Content::text(output.to_result_string())])),
            Err(e) => Ok(CallToolResult::error(vec![Content::text(e)])),
        }
    }

    #[tool(description = "Display file contents with syntax highlighting using bat (modern cat replacement). \
        Features: syntax highlighting, line numbers, git integration, line ranges.")]
    async fn bat(&self, Parameters(req): Parameters<BatRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec!["--color=never".into(), "--paging=never".into()];

        if req.number.unwrap_or(true) { args.push("-n".into()); }
        if req.show_all.unwrap_or(false) { args.push("-A".into()); }
        if let Some(ref lang) = req.language { args.push(format!("--language={}", lang)); }
        if let Some(ref range) = req.range { args.push(format!("--line-range={}", range)); }
        if let Some(ref highlight) = req.highlight { args.push(format!("--highlight-line={}", highlight)); }
        if let Some(ref style) = req.style { args.push(format!("--style={}", style)); }
    ...
    #[tool(description = "Find files and directories with fd (modern find replacement). \
        Features: regex patterns, respects gitignore, type filtering, parallel execution.")]
    async fn fd(&self, Parameters(req): Parameters<FdRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec!["--color=never".into()];

        if req.hidden.unwrap_or(false) { args.push("-H".into()); }
        if req.no_ignore.unwrap_or(false) { args.push("-I".into()); }
        if req.ignore_case.unwrap_or(false) { args.push("-i".into()); }
        if req.follow.unwrap_or(false) { args.push("-L".into()); }
        if req.absolute.unwrap_or(false) { args.push("-a".into()); }
        if let Some(ref ext) = req.extension { args.push(format!("-e{}", ext)); }
        if let Some(ref ft) = req.file_type { args.push(format!("-t{}", ft)); }
        if let Some(depth) = req.max_depth { args.push(format!("--max-depth={}", depth)); }
        if let Some(depth) = req.min_depth { args.push(format!("--min-depth={}", depth)); }
        if let Some(ref exclude) = req.exclude { args.push(format!("--exclude={}", exclude)); }
        if let Some(ref size) = req.size { args.push(format!("--size={}", size)); }
        if let Some(ref changed) = req.changed_within { args.push(format!("--changed-within={}", changed)); }
        if let Some(ref pattern) = req.pattern { args.push(pattern.clone()); }
        if let Some(ref path) = req.path { args.push(path.clone()); }
    ...
    async fn duf(&self, Parameters(req): Parameters<DufRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec![];

        if req.all.unwrap_or(false) { args.push("--all".into()); }
        if req.inodes.unwrap_or(false) { args.push("--inodes".into()); }
        if req.json.unwrap_or(false) { args.push("--json".into()); }
        if let Some(ref path) = req.path { args.push(path.clone()); }
    ...
    #[tool(description = "Analyze directory sizes with dust (modern du replacement). \
        Features: visual bars, tree view, customizable depth.")]
    async fn dust(&self, Parameters(req): Parameters<DustRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec![];

        if req.reverse.unwrap_or(false) { args.push("-r".into()); }
        if req.only_dirs.unwrap_or(false) { args.push("-D".into()); }
        if req.only_files.unwrap_or(false) { args.push("-F".into()); }
        if let Some(n) = req.number { args.push(format!("-n{}", n)); }
        if let Some(d) = req.depth { args.push(format!("-d{}", d)); }
        if let Some(ref path) = req.path { args.push(path.clone()); }
    ...
    async fn trash_put(&self, Parameters(req): Parameters<TrashRequest>) -> Result<CallToolResult, ErrorData> {
        match self.executor.run("trash-put", &[&req.path]).await {
            Ok(output) => Ok(CallToolResult::success(vec![Content::text(
                if output.success { format!("Moved '{}' to trash", req.path) } else { output.to_result_string() }
            )])),
            Err(e) => Ok(CallToolResult::error(vec![Content::text(e)])),
        }
    }
    ...
    async fn trash_empty(&self, Parameters(req): Parameters<TrashEmptyRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec![];
        if let Some(days) = req.days { args.push(format!("--days={}", days)); }

        let args_ref: Vec<&str> = args.iter().map(|s| s.as_str()).collect();
        match self.executor.run("trash-empty", &args_ref).await {
            Ok(output) => Ok(CallToolResult::success(vec![Content::text(
                if output.success { "Trash emptied".into() } else { output.to_result_string() }
            )])),
            Err(e) => Ok(CallToolResult::error(vec![Content::text(e)])),
        }
    }
    ...
    #[tool(description = "Search file contents with ripgrep (rg) - extremely fast grep replacement. \
        Features: regex, respects gitignore, parallel search, many output formats.")]
    async fn rg(&self, Parameters(req): Parameters<RipgrepRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec!["--color=never".into()];

        if req.ignore_case.unwrap_or(false) { args.push("-i".into()); }
        if req.smart_case.unwrap_or(false) { args.push("-S".into()); }
        if req.hidden.unwrap_or(false) { args.push("--hidden".into()); }
        if req.no_ignore.unwrap_or(false) { args.push("--no-ignore".into()); }
        if req.files_with_matches.unwrap_or(false) { args.push("-l".into()); }
        if req.count.unwrap_or(false) { args.push("-c".into()); }
        if req.line_number.unwrap_or(true) { args.push("-n".into()); }
        if req.word.unwrap_or(false) { args.push("-w".into()); }
        if req.invert.unwrap_or(false) { args.push("-v".into()); }
        if req.follow.unwrap_or(false) { args.push("-L".into()); }
        if req.multiline.unwrap_or(false) { args.push("-U".into()); }
        if req.only_matching.unwrap_or(false) { args.push("-o".into()); }
        if req.fixed_strings.unwrap_or(false) { args.push("-F".into()); }
        if req.json.unwrap_or(false) { args.push("--json".into()); }
        if let Some(ctx) = req.context { args.push(format!("-C{}", ctx)); }
        if let Some(ref ft) = req.file_type { args.push(format!("-t{}", ft)); }
        if let Some(ref glob) = req.glob { args.push(format!("--glob={}", glob)); }
        if let Some(max) = req.max_count { args.push(format!("-m{}", max)); }
        if let Some(ref replace) = req.replace { args.push(format!("--replace={}", replace)); }
    ...
    async fn fzf_filter(&self, Parameters(req): Parameters<FzfFilterRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec!["--filter".into(), req.query.clone()];

        if req.exact.unwrap_or(false) { args.push("-e".into()); }
        if req.ignore_case.unwrap_or(false) { args.push("-i".into()); }

        let args_ref: Vec<&str> = args.iter().map(|s| s.as_str()).collect();
        match self.executor.run_with_stdin("fzf", &args_ref, &req.input).await {
            Ok(output) => {
                let result = if let Some(limit) = req.limit {
                    output.stdout.lines().take(limit as usize).collect::<Vec<_>>().join("\n")
                } else {
                    output.stdout.clone()
                };
    ...
    #[tool(description = "Search code with ast-grep (sg) - AST-based structural search. \
        Find code patterns semantically, not just textually.")]
    async fn ast_grep(&self, Parameters(req): Parameters<AstGrepRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec!["run".into(), "--pattern".into(), req.pattern.clone()];

        if let Some(ref lang) = req.lang { args.push(format!("--lang={}", lang)); }
        if let Some(ref rewrite) = req.rewrite { args.push(format!("--rewrite={}", rewrite)); }
        if req.json.unwrap_or(false) { args.push("--json".into()); }
        if let Some(ref path) = req.path { args.push(path.clone()); }
    ...
    #[tool(description = "Find and replace text with sd (modern sed replacement). \
        Simpler syntax than sed, supports regex and literal modes.")]
    async fn sd(&self, Parameters(req): Parameters<SdRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec![];
        if req.fixed.unwrap_or(false) { args.push("-s".into()); }
        args.push(req.find.clone());
        args.push(req.replace.clone());

        let args_ref: Vec<&str> = args.iter().map(|s| s.as_str()).collect();
        match self.executor.run_with_stdin("sd", &args_ref, &req.input).await {
            Ok(output) => Ok(CallToolResult::success(vec![Content::text(output.to_result_string())])),
            Err(e) => Ok(CallToolResult::error(vec![Content::text(e)])),
        }
    }

    #[tool(description = "Process JSON with jq - the powerful command-line JSON processor. \
        Filter, transform, and query JSON data.")]
    async fn jq(&self, Parameters(req): Parameters<JqRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec![];

        if req.raw.unwrap_or(false) { args.push("-r".into()); }
        if req.compact.unwrap_or(false) { args.push("-c".into()); }
        if req.sort_keys.unwrap_or(false) { args.push("-S".into()); }
        if req.slurp.unwrap_or(false) { args.push("-s".into()); }
        args.push(req.filter.clone());

        let args_ref: Vec<&str> = args.iter().map(|s| s.as_str()).collect();
        match self.executor.run_with_stdin("jq", &args_ref, &req.input).await {
            Ok(output) => Ok(CallToolResult::success(vec![Content::text(output.to_result_string())])),
            Err(e) => Ok(CallToolResult::error(vec![Content::text(e)])),
        }
    }

    #[tool(description = "Process YAML/JSON/XML/CSV with yq - portable command-line processor. \
        Query and transform data in various formats.")]
    async fn yq(&self, Parameters(req): Parameters<YqRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec![];

        if req.prettyprint.unwrap_or(true) { args.push("-P".into()); }
        if let Some(ref fmt) = req.output_format { args.push(format!("-o={}", fmt)); }
        if let Some(ref fmt) = req.input_format { args.push(format!("-p={}", fmt)); }
        args.push(req.expression.clone());

        let args_ref: Vec<&str> = args.iter().map(|s| s.as_str()).collect();
        match self.executor.run_with_stdin("yq", &args_ref, &req.input).await {
            Ok(output) => Ok(CallToolResult::success(vec![Content::text(output.to_result_string())])),
            Err(e) => Ok(CallToolResult::error(vec![Content::text(e)])),
        }
    }
    ...
    async fn xsv(&self, Parameters(req): Parameters<XsvRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec![req.command.clone()];

        if req.no_headers.unwrap_or(false) { args.push("--no-headers".into()); }
        if let Some(ref d) = req.delimiter { args.push(format!("-d{}", d)); }
        ...
        let args_ref: Vec<&str> = args.iter().map(|s| s.as_str()).collect();
        match self.executor.run_with_stdin("qsv", &args_ref, &req.input).await {
            Ok(output) => Ok(CallToolResult::success(vec![Content::text(output.to_result_string())])),
            Err(e) => Ok(CallToolResult::error(vec![Content::text(e)])),
        }
    }

    #[tool(description = "Extract fields with hck (hack) - a faster cut replacement. \
        Extract columns from delimited text.")]
    async fn hck(&self, Parameters(req): Parameters<HckRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec!["-f".into(), req.fields.clone()];

        if let Some(ref d) = req.delimiter { args.push(format!("-d{}", d)); }
        if let Some(ref d) = req.output_delimiter { args.push(format!("-D{}", d)); }

        let args_ref: Vec<&str> = args.iter().map(|s| s.as_str()).collect();
        match self.executor.run_with_stdin("hck", &args_ref, &req.input).await {
            Ok(output) => Ok(CallToolResult::success(vec![Content::text(output.to_result_string())])),
            Err(e) => Ok(CallToolResult::error(vec![Content::text(e)])),
        }
    }
    ...
    #[tool(description = "List and filter processes with procs (modern ps replacement). \
        Features: tree view, sorting, filtering, colorful output.")]
    async fn procs(&self, Parameters(req): Parameters<ProcsRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec!["--color=never".into()];

        if req.tree.unwrap_or(false) { args.push("--tree".into()); }
        if let Some(ref sort) = req.sort { args.push(format!("--sortd={}", sort)); }
        if let Some(ref keyword) = req.keyword { args.push(keyword.clone()); }
    ...
    #[tool(description = "Count lines of code with tokei (fast code statistics). \
        Recognizes 150+ languages, shows code, comments, blanks.")]
    async fn tokei(&self, Parameters(req): Parameters<TokeiRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec![];

        if req.files.unwrap_or(false) { args.push("--files".into()); }
        if req.hidden.unwrap_or(false) { args.push("--hidden".into()); }
        if let Some(ref output) = req.output { args.push(format!("--output={}", output)); }
        if let Some(ref sort) = req.sort { args.push(format!("--sort={}", sort)); }
        if let Some(ref exclude) = req.exclude { args.push(format!("--exclude={}", exclude)); }
        if let Some(ref langs) = req.languages { args.push(format!("--types={}", langs)); }
        if let Some(ref path) = req.path { args.push(path.clone()); }
    ...
    async fn hyperfine(&self, Parameters(req): Parameters<HyperfineRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec!["--style=basic".into()];

        if req.json.unwrap_or(false) { args.push("--export-json=/dev/stdout".into()); }
        if let Some(warmup) = req.warmup { args.push(format!("--warmup={}", warmup)); }
        if let Some(min) = req.min_runs { args.push(format!("--min-runs={}", min)); }
        args.push(req.command.clone());
        if let Some(ref compare) = req.compare { args.push(compare.clone()); }
    ...
    async fn system_info(&self) -> Result<CallToolResult, ErrorData> {
        ...
        Ok(CallToolResult::success(vec![Content::text(
            if result.is_empty() { "Could not retrieve system info".into() } else { result }
        )]))
    }
    ...
    async fn http(&self, Parameters(req): Parameters<HttpRequest>) -> Result<CallToolResult, ErrorData> {
        ...
        if req.follow.unwrap_or(true) { args.push("--follow".into()); }
        if req.json_output.unwrap_or(false) { args.push("--json".into()); }
        if req.form.unwrap_or(false) { args.push("--form".into()); }
        if let Some(ref auth) = req.auth { args.push(format!("--auth={}", auth)); }
        if let Some(ref bearer) = req.bearer { args.push(format!("--bearer={}", bearer)); }
        if let Some(timeout) = req.timeout { args.push(format!("--timeout={}", timeout)); }
        if let Some(ref print) = req.print { args.push(format!("--print={}", print)); }
    ...
    #[tool(description = "DNS lookup with dog (modern dig replacement) or dig fallback. \
        Features: colorful output, DNS over HTTPS/TLS, multiple record types.")]
    async fn dns(&self, Parameters(req): Parameters<DnsRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec![req.domain.clone()];

        if let Some(ref rt) = req.record_type { args.push(rt.clone()); }
        if req.short.unwrap_or(false) { args.push("--short".into()); }
        if req.json.unwrap_or(false) { args.push("--json".into()); }
        if let Some(ref server) = req.server { args.push(format!("@{}", server)); }
        ...
        match self.executor.run("doggo", &args_ref).await {
            ...
            Err(e) => {
                ...
                if e.contains("not found") {
                    ...
                    if let Some(ref rt) = req.record_type { dig_args.push(rt); }
                    dig_args.push("+short");
    ...
    async fn usql(&self, Parameters(req): Parameters<UsqlRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec![req.url.clone()];

        if let Some(ref fmt) = req.format { args.push(format!("--format={}", fmt)); }
    ...
    async fn delta(&self, Parameters(req): Parameters<DeltaRequest>) -> Result<CallToolResult, ErrorData> {
        let diff_result = self.executor.run("diff", &["-u", &req.file_a, &req.file_b]).await;

        match diff_result {
            Ok(output) => {
                let mut delta_args = vec!["--color-only"];
                if req.side_by_side.unwrap_or(false) { delta_args.push("--side-by-side"); }
                if req.line_numbers.unwrap_or(true) { delta_args.push("--line-numbers"); }

                match self.executor.run_with_stdin("delta", &delta_args, &output.stdout).await {
                    Ok(delta_output) => Ok(CallToolResult::success(vec![Content::text(delta_output.to_result_string())])),
                    Err(_) => Ok(CallToolResult::success(vec![Content::text(output.to_result_string())])),
                }
            }
            Err(e) => Ok(CallToolResult::error(vec![Content::text(e)])),
        }
    ...
    async fn git_diff(&self, Parameters(req): Parameters<GitDiffRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec!["diff".into(), "--color=never".into()];

        if req.staged.unwrap_or(false) { args.push("--staged".into()); }
        if let Some(ref commit) = req.commit { args.push(commit.clone()); }
        if let Some(ref range) = req.range { args.push(range.clone()); }
        ...
        match self.executor.run_in_dir("git", &args_ref, path).await {
            Ok(output) => {
                match self.executor.run_with_stdin("delta", &["--color-only"], &output.stdout).await {
                    Ok(delta_output) => Ok(CallToolResult::success(vec![Content::text(delta_output.to_result_string())])),
                    Err(_) => Ok(CallToolResult::success(vec![Content::text(output.to_result_string())])),
                }
            }
    ...
    async fn bats(&self, Parameters(req): Parameters<BatsRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec![];

        if req.tap.unwrap_or(false) { args.push("--tap".into()); } else { args.push("--pretty".into()); }
        if req.count.unwrap_or(false) { args.push("--count".into()); }
        if let Some(ref filter) = req.filter { args.push(format!("--filter={}", filter)); }
        args.push(req.path.clone());
    ...
    async fn file_type(&self, Parameters(req): Parameters<FileTypeRequest>) -> Result<CallToolResult, ErrorData> {
    ...
    async fn permissions(&self, Parameters(req): Parameters<PermissionsRequest>) -> Result<CallToolResult, ErrorData> {
        let mode = &req.mode;
        let result = if mode.chars().all(|c| c.is_ascii_digit()) {
            ...
            format!(
                ...
                describe_perms(&user), describe_perms(&group), describe_perms(&other)
            )
        } else {
    ...
    async fn tldr(&self, Parameters(req): Parameters<TldrRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec![];
    ...
    async fn grex(&self, Parameters(req): Parameters<GrexRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec![];

        if req.escape.unwrap_or(false) { args.push("-e".into()); }
        if req.ignore_case.unwrap_or(false) { args.push("-i".into()); }
        if req.verbose.unwrap_or(false) { args.push("-x".into()); }
        if req.no_capture.unwrap_or(false) { args.push("-c".into()); }
        if req.anchors.unwrap_or(false) { args.push("-a".into()); }

        // Split input by newlines or commas
        let inputs: Vec<&str> = req.input
            .split(|c| c == '\n' || c == ',')
            .map(|s| s.trim())
            .filter(|s| !s.is_empty())
            .collect();
    ...
    async fn sad(&self, Parameters(req): Parameters<SadRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec![];

        if req.preview.unwrap_or(true) { args.push("--preview".into()); }
        if req.fixed.unwrap_or(false) { args.push("-f".into()); }
        if req.ignore_case.unwrap_or(false) { args.push("-i".into()); }
    ...
    async fn difft(&self, Parameters(req): Parameters<DifftasticRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec!["--color=never".into()];
    ...
    async fn ouch_compress(&self, Parameters(req): Parameters<OuchCompressRequest>) -> Result<CallToolResult, ErrorData> {
        ...
        match self.executor.run("ouch", &args_ref).await {
            Ok(output) => Ok(CallToolResult::success(vec![Content::text(
                if output.success { format!("Compressed to: {}", req.output) } else { output.to_result_string() }
            )])),
            Err(e) => Ok(CallToolResult::error(vec![Content::text(e)])),
        }
    }
    ...
    async fn ouch_decompress(&self, Parameters(req): Parameters<OuchDecompressRequest>) -> Result<CallToolResult, ErrorData> {
    ...
    async fn ouch_list(&self, Parameters(req): Parameters<OuchListRequest>) -> Result<CallToolResult, ErrorData> {
    ...
    async fn pueue_add(&self, Parameters(req): Parameters<PueueAddRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec!["add".into()];

        if req.immediate.unwrap_or(false) { args.push("--immediate".into()); }
        if req.stashed.unwrap_or(false) { args.push("--stashed".into()); }
    ...
    async fn pueue_status(&self, Parameters(req): Parameters<PueueStatusRequest>) -> Result<CallToolResult, ErrorData> {
    ...
    async fn pueue_log(&self, Parameters(req): Parameters<PueueLogRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec!["log".into()];

        if req.full.unwrap_or(false) { args.push("--full".into()); }
    ...
    async fn navi(&self, Parameters(req): Parameters<NaviRequest>) -> Result<CallToolResult, ErrorData> {
        let mut args: Vec<String> = vec![];
...
fn describe_perms(rwx: &str) -> String {
    ...
    if perms.is_empty() { "no permissions".into() } else { perms.join(", ") }
}
